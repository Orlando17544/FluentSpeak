...
class ConversationFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.recordButton.setOnClickListener {
            when(currentRecordingState) {
                RECORDING_STATE.START -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    recorder.pause()
                ...
                RECORDING_STATE.STOP -> {
                    recorder.start()
                    (it as MaterialButton).icon = resources.getDrawable(R.drawable.baseline_pause_24, null)
        ...
        binding.stopButton.setOnClickListener {
            when(currentRecordingState) {
                RECORDING_STATE.START, RECORDING_STATE.PAUSE -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    recorder.stop()
                    configureRecorder()
                    it.setEnabled(false)
                    (binding.recordButton as MaterialButton).icon = resources.getDrawable(R.drawable.baseline_mic_24, null)
                    binding.translateButton.setEnabled(true)
                ...
                RECORDING_STATE.STOP -> {
                    when(currentTranslatingState) {
                        TRANSLATING_STATE.STOP -> {
                            translator.start()
                            (binding.translateButton as MaterialButton).icon = resources.getDrawable(R.drawable.baseline_stop_24, null)
                        ...
                        TRANSLATING_STATE.START -> {
                            translator.stop()
                            configureTranslator()
                            (binding.translateButton as MaterialButton).icon = resources.getDrawable(R.drawable.baseline_translate_24, null)
        ...
        val requestPermissionLauncher =
            registerForActivityResult(
                ...
                if (isGranted) {
                    ...
                    // decision.
                    Snackbar.make(this.requireView(), "Voice recording does not work without the permission", Snackbar.LENGTH_SHORT).show()
                }
            }
    ...
    private fun configureRecorder() {
        recorder.apply {
            ...
            File.createTempFile("recording", null, context?.cacheDir)
            val cacheFile = File(context?.cacheDir, "recording")
    ...
    private fun configureTranslator() {
        translator.apply {
            ...
            File.createTempFile("translation", null, context?.cacheDir)
            val cacheFile = File(context?.cacheDir, "translation")
